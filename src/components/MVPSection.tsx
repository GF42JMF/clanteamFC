...
import { MATCHES_STORAGE_KEY, MATCH_HISTORY, ASSETS } from '../constants';
...
const MVPSection: React.FC<MVPSectionProps> = ({ role, currentUser, players }) => {
  ...
  const eligibleIds: string[] = lastMatch?.eligiblePlayerIds || [];
  const eligiblePlayers: Player[] = players.filter((player: Player) => eligibleIds.includes(player.id));
  ...
  const results = useMemo((): Array<{ player: Player; votes: number }> => {
    if (!lastMatch) return [];
    const votes: Record<string, number> = lastMatch.votes || {};
    return eligiblePlayers
      .map((player: Player) => ({ player, votes: votes[player.id] || 0 }))
